...
class JohndownParser {
  ...
  parse(johndown) {
    ...
    // First pass: preprocess inline footnotes [$content] -> [^INLINENOTE#]
          ...
          while (j < text.length) {
            if (text[j] === "[") {
              ...
              if (bracketDepth === 0) {
                ...
                const autoId = `^INLINENOTE${++inlineCounter}`;
  ...
  processInlineStyling(text) {
    ...
    text = text.replace(/\[\:lang\:(\S*)\s*([^\]]*?)]/g, "<span lang=\"$1\">$2</span>");
    //Slight DRY violation in the below:
    text = text.replace(/\[\:ilang\:(\S*)\s*([^\]]*?)]/g, "<span style=\"font-style:italic\" lang=\"$1\">$2</span>");
  ...
  formatFootnotes() {
    ...
    for (const footnote of this.footnoteOrder) {
      ...
      const pprocessedppContentp = this.convertToHtml(footnote.content)
      const processedContent = pprocessedppContentp.slice(3); //slice 3 gets rid of the leading <p>, which we don't want here.
      footnotesHtml += `
